...
if __name__ == "__main__":
    ...
    print(basis)
    mesh = DGMesh1D.build_from_equidistant_grid(x_min=0, x_max=1.0, num_cells=100)
    print(mesh)
    sod = lambda x: np.where(x < 0.5, np.array([1.0, 0.0, 1.0 / 0.4]), np.array([0.125, 0.0, 0.1 / 0.4]))
    field = DGField1D(basis, mesh, sod)
    print(field)
    system = DGSystem1D(field, SystemOperator1D(), stepper.SSPRK3Stepper())
    print(system)
