...
class RungeKuttaStepperForDITR(ExplicitStepper):
    """Base class for explicit Runge-Kutta steppers for DITR."""
    ...
    def __init__(
        ...
        """
        Init function for Runge-Kutta steppers for DITR.

        Args
        ----
        F_ref : tensor
            The reference tensor of F for shape from DITRStepper.
        rtol : float
    ...
    def _wrap_y_as_u(self, u: Tuple[tensor], y: tensor) -> Tuple[tensor]:
        return (*u[:-2], y[0, ...], y[1, ...])

    @abstractmethod
    def step(
        self, u: Tuple[tensor], dt: tensor, rhs: Callable[[tensor], tensor], *, out: Optional[tensor] = None
    ) -> tensor:
        """
        The form of u must be same as DITRStepper.
        The rhs here is the trhs from DITRStepper.
        """
...
class DITRU2R2Stepper(DITRStepper):
    ...
    def trhs(self, u: Tuple[tensor], dt: float) -> tensor:
        ...
        The form of u is (u_{n}, u_{n+c2}, u_{n+1})
        """
